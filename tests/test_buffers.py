...
from underfit_api.models import LogLine, Scalar
...
def test_log_ingest_validates_start_line_and_round_trips_buffered(engine: Engine) -> None:
    ...
    with engine.begin() as conn:
        log_buffer.append(conn, rwid, 0, [LogLine(timestamp=T0, content="a"),
                                          LogLine(timestamp=T0, content="b")])
        assert log_buffer.end_line(conn, rwid) == 2
        with pytest.raises(BadStartLineError) as exc:
            log_buffer.append(conn, rwid, 1, [LogLine(timestamp=T0, content="late")])
        assert exc.value.expected == 2
        entries = log_buffer.read_buffered(conn, rwid, cursor=0, count=10)
...
def test_log_compaction_writes_segment_and_clears_chunks(engine: Engine, tmp_path: Path) -> None:
    ...
    with engine.begin() as conn:
        log_buffer.append(conn, rwid, 0, [LogLine(timestamp=T0, content="a")])
        log_buffer.append(conn, rwid, 1, [LogLine(timestamp=T0, content="b")])
...
def test_log_compaction_skips_below_byte_threshold_until_worker_stale(engine: Engine, tmp_path: Path) -> None:
    rwid = _create_worker(engine)
    storage = FileStorage(FileStorageConfig(base=str(tmp_path / "storage")))
    config.buffer.log_segment_bytes = 100
    with engine.begin() as conn:
        log_buffer.append(conn, rwid, 0, [LogLine(timestamp=T0, content="x")])
    log_buffer.compact(engine, storage)
    with engine.begin() as conn:
...
def test_scalar_ingest_validates_step_and_start_line(engine: Engine) -> None:
    rwid = _create_worker(engine)
    with engine.begin() as conn:
        scalar_buffer.append(conn, rwid, 0, _scalars(0, 3))
        with pytest.raises(BadStartLineError) as exc:
            scalar_buffer.append(conn, rwid, 0, _scalars(10, 1))
        assert exc.value.expected == 3
        with pytest.raises(BadStepError) as step_exc:
            scalar_buffer.append(conn, rwid, 3, [Scalar(step=2, values={"loss": 0.0}, timestamp=T0)])
...
def test_scalar_buffered_read_downsamples_live_tail(engine: Engine) -> None:
    rwid = _create_worker(engine)
    with engine.begin() as conn:
        scalar_buffer.append(conn, rwid, 0, _scalars(0, 10))
        r1 = scalar_buffer.read_buffered(conn, rwid, 1)
        r10 = scalar_buffer.read_buffered(conn, rwid, 10)
        assert len(r1) == 10
        assert [p.values["loss"] for p in r10] == [4.5]
        assert scalar_buffer.end_line(conn, rwid, 10) == 1
...
def test_scalar_compaction_full_chunk_emits_all_resolutions(engine: Engine, tmp_path: Path) -> None:
    ...
    with engine.begin() as conn:
        scalar_buffer.append(conn, rwid, 0, _scalars(0, 25))
...
def test_scalar_partial_flush_waits_for_entire_run_to_go_quiet(engine: Engine, tmp_path: Path) -> None:
    rwid_a = _create_worker(engine)
    storage = FileStorage(FileStorageConfig(base=str(tmp_path / "storage")))
    with engine.begin() as conn:
        worker_a = workers_repo.get_by_id(conn, rwid_a)
        assert worker_a is not None
        workers_repo.create(conn, worker_a.run_id, "1")
        scalar_buffer.append(conn, rwid_a, 0, _scalars(0, 7))
        conn.execute(run_workers.update().where(run_workers.c.id == rwid_a).values(
            last_heartbeat=T0 - timedelta(seconds=config.buffer.worker_timeout_s + 1),
        ))
    scalar_buffer.compact(engine, storage)
    with engine.begin() as conn:
        assert conn.execute(select(scalar_segments).where(scalar_segments.c.worker_id == rwid_a)).all() == []


def test_scalar_compaction_skips_below_threshold_until_worker_stale(engine: Engine, tmp_path: Path) -> None:
    rwid = _create_worker(engine)
    storage = FileStorage(FileStorageConfig(base=str(tmp_path / "storage")))
    config.buffer.scalar_resolutions = [1, 10]
    config.buffer.scalar_segment_lines = 100
    with engine.begin() as conn:
        scalar_buffer.append(conn, rwid, 0, _scalars(0, 7))
    scalar_buffer.compact(engine, storage)
    with engine.begin() as conn:
        assert conn.execute(select(scalar_segments).where(scalar_segments.c.worker_id == rwid)).all() == []
        conn.execute(run_workers.update().where(run_workers.c.id == rwid).values(
            last_heartbeat=T0 - timedelta(seconds=config.buffer.worker_timeout_s + 1),
        ))
    scalar_buffer.compact(engine, storage)
