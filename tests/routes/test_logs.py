...
def test_write_read_and_flush_logs(
    ...
    assert (
        first_json["entries"][0]["content"] == "hello\nworld"
        and first_json["nextCursor"] == 2
...
def test_logs_reject_out_of_order_start_line(
    ...
    duplicate = client.post(logs_url, headers=headers, json={**payload, "start_line": 0, "lines": [world_line]})
