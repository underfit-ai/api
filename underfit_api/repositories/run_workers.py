...
def create(conn: Connection, run_id: UUID, worker_id: str, status: str, is_primary: bool) -> Worker:
    row_id = uuid4()
    now = utcnow()
    conn.execute(run_workers.insert().values(
        id=row_id, run_id=run_id, worker_id=worker_id, status=status, joined_at=now,
    ))
    if is_primary:
        conn.execute(runs.update().where(runs.c.id == run_id).values(primary_worker_id=row_id))
    return Worker(id=row_id, run_id=run_id, worker_id=worker_id, is_primary=is_primary, status=status, joined_at=now)
...
def get(conn: Connection, run_id: UUID, worker_id: str) -> Worker | None:
    row = conn.execute(
        sa.select(*_columns).select_from(_join)
        .where(run_workers.c.run_id == run_id, run_workers.c.worker_id == worker_id),
    ).first()
    return Worker.model_validate(row) if row else None


def update_status(conn: Connection, run_id: UUID, worker_id: str, status: str) -> Worker | None:
    conn.execute(
        run_workers.update()
        .where(run_workers.c.run_id == run_id, run_workers.c.worker_id == worker_id)
        .values(status=status),
    )
    return get(conn, run_id, worker_id)
