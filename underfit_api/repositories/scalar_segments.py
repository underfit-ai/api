...
def upsert(
    ...
    updates = {"end_line": end_line, "end_step": end_step, "end_at": end_at, "storage_key": storage_key}
    conn.execute(dialect_insert(conn, scalar_segments).values(
        id=uuid4(), worker_id=worker_id, resolution=resolution, start_line=start_line,
        start_at=start_at, created_at=utcnow(), **updates,
