...
**Database** — SQLite is the default and requires no configuration. PostgreSQL and MySQL are also supported; install `underfit-api[psql]` or `underfit-api[mysql]`, then set `[database] type` to `"postgresql"` or `"mysql"` and provide `host`, `port`, `user`, `password`, and `database`.
