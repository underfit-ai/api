...
from app.repositories import collaborators as collaborators_repo
from app.repositories import organizations as organizations_repo
from app.schema import accounts, projects


def _project_account(conn: Connection, project_id: uuid.UUID) -> tuple[uuid.UUID, str]:
    row = conn.execute(
...
def require_account_admin(conn: Connection, account_id: uuid.UUID, account_type: str, user_id: uuid.UUID) -> None:
...
def require_project_admin(conn: Connection, account_id: uuid.UUID, account_type: str, user_id: uuid.UUID) -> None:
...
def _project_info(conn: Connection, project_id: uuid.UUID) -> tuple[str, uuid.UUID, str]:
    row = conn.execute(
...
def require_project_viewer(conn: Connection, project_id: uuid.UUID, user_id: uuid.UUID | None) -> None:
...
def can_view_project(conn: Connection, project_id: uuid.UUID, user_id: uuid.UUID | None) -> bool:
