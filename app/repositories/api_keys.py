...
from app.models import ApiKey, ApiKeyWithToken
from app.schema import api_keys


def list_by_user(conn: Connection, user_id: uuid.UUID) -> list[ApiKey]:
    rows = conn.execute(api_keys.select().where(api_keys.c.user_id == user_id)).all()
    return [ApiKey.model_validate(r) for r in rows]


def create(conn: Connection, user_id: uuid.UUID, label: str | None) -> ApiKeyWithToken:
    key_id = uuid.uuid4()
...
def delete(conn: Connection, key_id: uuid.UUID, user_id: uuid.UUID) -> bool:
